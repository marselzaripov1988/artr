...
//
// Ноль: механизм по умолчанию выключен, и это решение, а не недоделка.
//
// Сеть прожила годы без экономического наказания. В выгрузке мейннета
// одиннадцать валидаторов из 1740 имеют записанные нарушения консенсуса
// и ни один не дошёл до второго — это почерк не атаки, а житейской
// оплошности: оператор держит запасной узел с копией ключа, случается
// разрыв, обе машины подписывают. Отнимать деньги за такое, когда и без
// того закрывается право валидировать, несоразмерно.
//
// Включать имеет смысл перед открытием публичного канала IBC, через
// который пойдёт заметная ценность: там светлый клиент чужой сети
// доверяет заголовкам Artery, исходя из того, что за подделку можно
// наказать. До тех пор отсутствие наказания — внутреннее дело сети.
//
// Включение — одно число через голосование, правки кода не требует.
var DefaultMisbehaviourPenalty = util.Percent(0)
