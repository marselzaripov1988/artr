...
func NewArteryApp(
	...
	app.accountKeeper = authKeeper.NewAccountKeeper(
		...
		map[string][]string{
			...
			// Счёт модуля noding — казна для средств, изъятых за
			// византийское поведение. Прав чеканить и жечь ему не нужно:
			// деньги не создаются и не исчезают, а меняют владельца.
			//
			// Заводится здесь, а не в maccPerms: та карта в этой сборке не
			// используется вовсе, счётный кеепер берёт права из этого
			// списка. На том же уже спотыкался модуль переводов IBC.
			noding.ModuleName: {},
		},
