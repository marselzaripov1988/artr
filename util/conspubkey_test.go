...
//
// Кодировка консенсусных ключей взята из SDK себе, потому что пакет
// legacybech32 удаляется в 0.50. Формат при этом менять нельзя: им
// записаны генезис и стор работающей сети. Пока обе реализации
// существуют рядом, сверяем их на случайных ключах — после подъёма SDK
// сверять будет уже не с чем.
